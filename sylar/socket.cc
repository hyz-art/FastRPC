...
bool Socket::getOption(int level, int option, void *result, socklen_t *len){
    ...
    if (rt) {
        SYLAR_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock << " level=" << level << " option=" << option << ", error" << errno
...
Socket::ptr Socket::accept(){
    Socket::ptr socket(new Socket(m_family, m_type, m_protocol));
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    }else if(::connect_with_timeout(m_sock, addr->getAddr(), addr->getAddrLen(), timeout_ms)){
        SYLAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::reconnect(const Address::ptr addr, uint64_t timeout_ms) {
    if(!m_remoteAddress){
...
int Socket::recv(void* buffers, size_t length, int flags) {
    if(isConnected()){
        return ::recv(m_sock,buffers,length,flags);
    }
    return -1;
}
...
Address::ptr Socket::getRemoteAddress() {
    if (m_remoteAddress) {//获取地址
        return m_remoteAddress;
    }

    Address::ptr result;
    switch (m_family) {//创造对象
...
void Socket::newSock() {
    ...
    if(SYLAR_LIKELY(m_sock!=-1)){
        initSock();        
    }else{SYLAR_LOG_ERROR(g_logger)<<"socket("<<m_family<<","<<m_type<<","<<m_protocol<<") errno="<<errno
            <<" errstr="<<strerror(errno);}
}
//绑定文件描述符
