...
static T CreateMask(uint32_t bits){
    return (1<<(sizeof(T)*8-bits))-1;//获取掩码，通过移位-1，
}
template <class T>
...
// Specialization for in6_addr
// static uint32_t CountBytes(const in6_addr& addr) {
//     uint32_t result = 0;
//     for (int i = 0; i < 16; ++i) {
...
bool Address::GetInterfaceAddresses(std::vector<std::pair<Address::ptr,uint32_t>>& result,
    ...
    if(iface.empty()||iface=="*"){
        if(family == AF_INET || family == AF_UNSPEC){//设为空
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    hints.ai_flags=AI_NUMERICHOST;//不解码
    hints.ai_family=AF_UNSPEC;//两种类型ip
    int error=getaddrinfo(address,0,&hints,&results);
    if(error){
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        SYLAR_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        SYLAR_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", " << port << ") rt=" << result
                                  << " errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port) {
    ...
    memcpy(&m_addr.sin6_addr.s6_addr,address,16);//存储数组16位
}
...
std::ostream& IPv6Address::insert(std::ostream& os) const {
    ...
    //inet_ntop(AF_INET6, &m_addr.sin6_addr, str, sizeof(str));
    uint16_t host_addr[8];
    for (int i = 0; i < 8; ++i) {
        host_addr[i] = byteswapOnLittleEndian(addr[i]);
    }
    ...
    bool printed_colon_colon = false;
    for (int i = 0; i < 8; ++i) {
        if (best_start != -1 && i == best_start) {
            os << "::";
            printed_colon_colon = true;
            i += best_len - 1;
            continue;
        }
        if (i > 0 && !(printed_colon_colon && i <= best_start + best_len)) {
            os << ":";
        }
        if (host_addr[i] != 0 || best_start == -1) {
            os << std::hex << host_addr[i] << std::dec;
        }
    }
...
IPAddress::ptr IPv6Address::broadcastAddress(uint32_t prefix_len) {
    ...
    for (int i = prefix_len / 8 + 1; i < 16; ++i) {//尾部位置设为0xff
...
UnixAddress::UnixAddress() {
    ...
    //获取结构体中某个字段相对于结构体开头的字节偏移量
    m_length=offsetof(sockaddr_un, sun_path)+MAX_PATH_LEN;
}
UnixAddress::UnixAddress(const std::string& path) {
    memset(&m_addr, 0, sizeof(m_addr));
    m_addr.sun_family = AF_UNIX;
    m_length=sizeof(path)+1;
    //根据抽象字符串判断
