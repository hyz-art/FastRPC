...
static sylar::ConfigVar<uint64_t>::ptr g_tcp_connect_timeout =
    sylar::Config::Lookup<uint64_t>("tcp.connect.timeout", 5000, "tcp connect timeout");
    ...
    _HookIniter()
    {
        ...
        g_tcp_connect_timeout->addListener([](const int &old_value, const int &new_value){
            s_connect_timeout=new_value;
                    SYLAR_LOG_INFO(g_logger)<<"hook init connect timeout="<<s_connect_timeout;
        });
    }
};
...
static ssize_t do_io(int fd, OriginFun fun, const char *hook_fun_name, uint32_t event, int timeout_so, Args &&...args)
{
    ...
    // 用户设置了非阻塞 / 非 socket 的情况
    if (!ctx->isSocket() || ctx->getUserNonblock()){
        return fun(fd, std::forward<Args>(args)...);
    }

    uint64_t to = ctx->getTimeout(timeout_so);
    std::shared_ptr<timer_info> tinfo(new timer_info); // 用于后续控制定时器状态
    ...
    if (n == -1 && errno == EAGAIN)
    {
        ...
        {
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << hook_fun_name << " addEvent(" << fd << "," << event << ")";
            if (timer)
            {
                timer->cancel();
            }
        ...
        else
        {
            ...
            // addConditionTimer构造了带定时器的回调
            if (timer)
            { // 定时器取消或定时器超时
                timer->cancel();
            }
            // 判断当前是否是正常 I/O 唤醒，还是超时唤醒
            if (tinfo->cancelled)
            { // 若cancelled不为0，表示超时
...
int connect_with_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms){
    ...
    if(rt==0){
        ...
        SYLAR_LOG_ERROR(SYLAR_LOG_ROOT())<<"connect addEvent("<<fd<<","<<")";
        if(timer){
            timer->cancel();
        }
    }
